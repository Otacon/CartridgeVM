...
class BusTest {
    ...
    @Test fun cartridgePrgMirroring() { val prg = ByteArray(16 * 1024); prg[0] = 0x12; val c = Cartridge(0, Mirroring.HORIZONTAL, prg, ByteArray(8192), true, false); assertEquals(0x12, c.mapper.cpuRead(0x8000)); assertEquals(0x12, c.mapper.cpuRead(0xC000)) }
    ...
    @Test fun oamDmaRouting() { val cart = Cartridge(0, Mirroring.HORIZONTAL, ByteArray(16 * 1024), ByteArray(8192), true, false); val ppu = Ppu(PpuBus(cart.mapper, cart.mirroring)); val bus = CpuBus(cart, ppu, NesController(), NesApu()); bus.write(0x0000, 0x77); bus.write(0x4014, 0); assertEquals(0x77, ppu.oam[0].toInt() and 0xFF); assertTrue(bus.consumeDmaCycles() >= 513) }

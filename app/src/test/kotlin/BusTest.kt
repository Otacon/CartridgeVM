...
class BusTest {
    ...
    @Test fun oamDmaRouting() { val cart = Cartridge(0, Mirroring.HORIZONTAL, ByteArray(16 * 1024), ByteArray(8192), true, false); val ppu = Ppu(Mapper0(cart), Mirroring.HORIZONTAL); val bus = CpuBus(Mapper0(cart), ppu, NesController()); bus.write(0x0000, 0x77); bus.write(0x4014, 0); assertEquals(0x77, ppu.oam[0].toInt() and 0xFF); assertTrue(bus.consumeDmaCycles() >= 513) }
}
